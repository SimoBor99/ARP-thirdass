	...
	char pn[10];
  	...
  	if (decision[0] == '1' && strlen(decision) == 2) {
		...
		sprintf(pn, "%d", port_number);
  		...
  		char * arg_list_A[] = { "/usr/bin/konsole", "-e", "./bin/processA_client", host, pn, NULL };
		...
		sprintf(pn, "%d", port_number);
  		...
  		// define list of arguments of processes
  		char * arg_list_A[] = { "/usr/bin/konsole", "-e", "./bin/processA_server", shm_name, pn, NULL };
