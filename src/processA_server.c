...
int main(int argc, char *argv[]) {	
    ...
    while (TRUE) {
        ...
        n = read(newsockfd,&tmp1,sizeof(tmp1));
		tmp2=ntohl(tmp1);
		write_log("tt\n", "oo.txt");
        if (n < 0)  {
		...
		if(tmp2==KEY_LEFT || tmp2==KEY_DOWN || tmp2==KEY_RIGHT || tmp2==KEY_UP || tmp2==KEY_MOUSE) {
