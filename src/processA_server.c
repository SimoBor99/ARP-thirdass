...
int main(int argc, char *argv[]) {	
    ...
    sockfd = socket(AF_INET, SOCK_STREAM, 0);
	 ...
	 // link the server to a port number
    ...
    while (TRUE) {
		...
		// check is someone has connected
        if (!act_conn) {
    	...
    	if (strcmp(input, "disc")==0) {
            act_conn = 0;
        }

		// convert the input into interger number
		command=atoi(input);
		
		// it arrives one command from the client
		if(command==KEY_LEFT || command==KEY_DOWN || command==KEY_RIGHT || command==KEY_UP || command==KEY_MOUSE) {
